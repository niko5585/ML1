...
class MLPClassifierOwn():
    ...
    def softmax(z: List[Scalar]) -> List[Scalar]:
        ...
        """
        scalarListToReturn = list()
        scalarSumExp = Scalar(0)
        for scalar in z:
            scalarSumExp = scalarSumExp + scalar.exp()

        for currentScalar in z:
            scalarListToReturn.append(currentScalar.exp() / scalarSumExp)

        return scalarListToReturn
